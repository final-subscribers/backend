...
public class AuthServiceTest {
    ...
    @Mock private AuthSmsService authSmsService;
    ...
    void testAdminSignup_EmailDuplication() {
        // 어드민 이메일 중복
        AdminSignUpRequest request = AdminSignUpRequest.builder().email("john@example.com").build();

        when(adminRepository.existsByEmail(request.getEmail())).thenReturn(true);

        InvalidValueException thrown =
                assertThrows(
    ...
    void testMemberSignup_EmailDuplication() {
        // 사용자 이메일 중복
        MemberSignUpRequest request =
                MemberSignUpRequest.builder().email("jane@example.com").build();
    ...
    void testLogin_Admin_Success() {
        ...
        LoginRequest request =
                LoginRequest.builder().email("admin@example.com").password("password").build();
    ...
    void testLogin_Member_Success() {
        // 사용자 로그인 성공
        LoginRequest request =
                LoginRequest.builder().email("member@example.com").password("password").build();
    ...
    void testLogin_PasswordMismatch() {
        // 비밀번호 불일치 로그인 실패
        LoginRequest request =
                LoginRequest.builder().email("admin@example.com").password("wrongpassword").build();
            ...
            mockedCookieUtil.verify(
                    () ->
                            CookieUtil.addCookie(
                                    eq(response), eq("accessToken"), eq(accessToken), anyLong()),
                    times(1));
            mockedCookieUtil.verify(
                    () ->
                            CookieUtil.addCookie(
                                    eq(response), eq("refreshToken"), eq(refreshToken), anyLong()),
                    times(1));
        }
    }
    ...
    void testStandardLogout_Success() {
        ...
        verify(response)
                .addHeader(
                        "Set-Cookie",
                        "accessToken=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure; HttpOnly; SameSite=None");
        verify(response)
                .addHeader(
                        "Set-Cookie",
                        "refreshToken=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure; HttpOnly; SameSite=None");
    }
    ...
    void testDeleteTokenCookies() {
        ...
        try (MockedStatic<CookieUtil> mockedCookieUtil = mockStatic(CookieUtil.class)) {
            ...
            mockedCookieUtil.verify(
                    () -> CookieUtil.deleteCookie(eq(response), eq("refreshToken")), times(1));
        }
    }
}
